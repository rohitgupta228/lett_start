    ...
    if ($('#login-form').length) {
      $('#login-form').validate({
        ...
        submitHandler: function (form) {
          ...
          if($.API){
            var prom = $.API.postWithoutToken('user/login', payload);
            prom.then(function(data) {
              $(form)[0].reset();
              if (data && data.hasOwnProperty('token')) {
                var tokenOriginal = data.token.original;
                localStorage.setItem("token", tokenOriginal.access_token);
                window.location.reload();
              }
            }).catch(function(error) {
              if (error.responseJSON && error.responseJSON.code === 401) {
                $(form).find("#errorMsg").find('p').remove();
                $(form).find("#errorMsg").append("<p>Invalid username or password.</p>");
