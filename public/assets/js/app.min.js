!function ($) {
  var App = function () {
  }

  //init app
  App.prototype.init = function () {
    //end test code
    var pageUrl = window.location.href.split(/[?#]/)[0];
    this.hideLoader()
    var self = this;

    if ($('.accordion .collapse').length) {
      $('.accordion .collapse').on('show.bs.collapse', function () {
        $(this).parents(".card").addClass("active")
      })
      $('.accordion .collapse').on('hide.bs.collapse', function () {
        $(this).parents(".card").removeClass("active")
      })
    }

    if ($(".back-top").length) {
      $(".back-top").on("click", function () {
        $("html, body").animate({
          scrollTop: 0
        }, 500)
      })
    }

    $(".navbar-header .navbar-toggler").on("click", function () {
      var target = $(this).attr("data-target");
      $(target).slideToggle().removeClass("collapse");
    });

    //Change profile image click
    $("#choose-file").on("click", function () {
      $("#upload-image").trigger('click');
    });

    if ($('#resetModal').length) {
      $('#resetModal').on('hide.bs.modal', function () {
        window.history.pushState({}, "", window.location.origin + window.location.pathname);
      });
    }

    // loader
    $(".header .navbar-nav .nav-link").each(function () {
      if (this.href == pageUrl) {
        $(this).addClass("active");
      }
    });

    $(".password-group .icon-view").on("click", function () {
      $(this).parent().toggleClass('active');
      var input = $(this).siblings('input');
      if (input.attr('type').toLowerCase() === 'text') {
        input.attr('type', 'password');
      }
      else {
        input.attr('type', 'text');
      }
    });

    $('.modal').modal({ backdrop: 'static', keyboard: false, show: false })

    $("#forgot-btn").on("click", function () {
      $(".auth-modal").modal('hide');
      $("#forgotModal").modal('show');
    })

    $(".register-btn").on("click", function () {
      $(".auth-modal").modal('hide');
      $("#registerModal").modal('show');
    })

    $("#login-btn, #forgot-login-btn, #auth-btn").on("click", function () {
      $(".auth-modal").modal('hide');
      $("#loginModal").modal('show');
    })

    //mobile menu
    $(".header .navbar-nav .dropdown-item").each(function () {
      if (this.href == pageUrl) {
        $(this).addClass("active");
        $(this).parents('.dropdown-menu').siblings(".nav-link").addClass('active');
      }
    });

    //upload profile image
    $("#upload-image").on("change", function () {
      $("#choose-file").addClass("disable-events")
      var file = this.files[0];
      self.loadImg(file, this)
    })

    $("#search-form").on("submit", function () {
      var keyword = $(this).find(".form-control").val();
      self.searchString(keyword);
    })

    $(".keyword-list a").on("click", function () {
      var keyword = $(this).attr("data-search");
      self.searchString(keyword);
    })

    //init service worker
    this.initServiceWorker();

    //hide error
    this.messages();

    //window scroll method
    this.windowScroll();
  }

  App.prototype.searchString = function (keyword) {
    var param = new URLSearchParams();
    param.append("s", keyword);
    window.location.href = "/search-result?" + param;
  }

  //init service worker
  App.prototype.initServiceWorker = function () {
    if ('serviceWorker' in navigator) {
      navigator.serviceWorker.register(swUrl)
        .then(function (reg) {
          console.log('service worker registered')
        })
        .catch(function (err) {
          console.log('service worker not registered', err)
        });
    }
  }

  //close error msgs
  App.prototype.messages = function () {
    if ($(".error-msgs").length) {
      $(document).on('click', ".error-msgs .close", function () {
        $(this).parents(".error-msgs").find("p").remove();
        $(this).parent().hide();
      })
    }
    if ($(".success-msgs").length) {
      $(document).on('click', ".success-msgs .close", function () {
        $(this).parents(".success-msgs").find("p").remove();
        $(this).parent().hide();
      })
    }
  }


  //show profile image method
  App.prototype.loadImg = function (file, self) {
    var img, instance = this;
    var defaultImg = "assets/images/client-thumb.png";
    if (file) {
      var type = file.name.match(/.(jpg|jpeg|png|gif)$/i);
      if (!type) {
        instance.showError(self, "Please choose valid file", "alert-success", "alert-danger");
        $("#choose-file").removeClass("disable-events")
        return;
      }
      $(self).parents("form").find(".error-msgs").hide();
      img = new Image();
      img.onload = function () {
        if (this.width > 160) {
          var msg = "The file is too large. Allowed maximum size of 160 X 160";
          $(".profile-image img").hide();
          $(".profile-image img").attr("src", defaultImg).show()
          instance.showError(self, msg, "alert-success", "alert-danger");
          $("#choose-file").removeClass("disable-events")
          $("#choose-file").find("span").show();
          $("#choose-file").find(".btn-loader").hide();
          return;
        }
        else {
          instance.uploadImage();
        }
      };
      img.onerror = function () {
        error = {
          flag: true,
          msg: "Image load error"
        }
      };
      img.src = window.URL.createObjectURL(file);
      img.style.display = "none";
      $(".profile-image img").attr("src", window.URL.createObjectURL(file))
    }
  }

  //change profile image
  App.prototype.uploadImage = function () {
    $("#choose-file").removeClass("disable-events")
    $("#submit-btn").trigger('click');
  }

  //show error
  App.prototype.showError = function (currentElem, msg, removeClasses, addClasses) {
    $(currentElem).parents("form").find(".error-msgs").find("p").remove();
    $(currentElem).parents("form").find(".error-msgs").removeClass(removeClasses).addClass(addClasses).append("<p>" + msg + "</p>").fadeIn(200);
  }

  //show fakeloader
  App.prototype.showLoader = function () {
    $(".fakeLoader").show();
  }

  //hide fakeloader
  App.prototype.hideLoader = function () {
    $(".fakeLoader").hide();
  }

  //show fakeloader
  App.prototype.showContentLoader = function (elem) {
    $(elem).addClass('show');
  }

  //hide fakeloader
  App.prototype.hideContentLoader = function (elem) {
    $(elem).removeClass('show');
  }

  /**
   * ----------------------------------------------
   * Nav Scroll active section class 
   * ----------------------------------------------
   */
  App.prototype.windowScroll = function () {
    //get document height
    var docHeight = $(document).outerHeight();
    $(window).on('scroll', function () {
      var scrollPos = $(this).scrollTop();
      if (scrollPos >= 20) {
        $(".header").addClass("fixed-nav");
      } else {
        $(".header").removeClass("fixed-nav");
      }
      if (scrollPos > docHeight / 2) {
        $(".back-top").addClass("show");
      }
      else {
        $(".back-top").removeClass("show");
      }
    })
  }

  //init App
  $.App = new App, $.App.Constructor = App
}(window.jQuery),
  //initializing App
  function ($) {
    "use strict";
    $.App.init();
  }(window.jQuery);

!function ($) {
  var Auth = function () { };
  //init auth
  Auth.prototype.init = function () {
    this.validateLogin();
    this.validateRegister();
    this.validateForgot();
    this.validateReset();
  }
  //validate login form
  Auth.prototype.validateLogin = function () {
    if ($('#login-form').length) {
      $('#login-form').validate({
        focusInvalid: false,
        rules: {
          'username': {
            required: true
          },
          'password': {
            required: true,
          },
          'validation-required': {
            required: true
          }
        },
        errorPlacement: function errorPlacement(error, element) {
          $(element).siblings(".validation-error").removeClass("d-none");
          return true
        },
        highlight: function (element) {
          var $el = $(element);
          var $parent = $el.parents('.form-group');
          $parent.addClass("invalid-field");
        },
        unhighlight: function (element) {
          var $el = $(element);
          var $parent = $el.parents('.form-group');
          $parent.removeClass("invalid-field");
          $(element).siblings(".validation-error").addClass("d-none");
        },
        submitHandler: function (form) {
          $(form).find("[type = 'submit']").addClass("disable-events");
          if ($.API) {
            var prom = $.API.postWithoutToken(null, $(form).serializeArray(), loginRoute);
            prom.then(function (data) {
              $(form)[0].reset();
              window.location.reload()
            }).catch(function (error) {
              if (error && error.responseJSON) {
                var resp = error.responseJSON;
                if (resp.errors) {
                  var errorMsg = resp.errors;
                  $(form).find(".error-msgs p").remove();
                  for (var i in errorMsg) {
                    var pelem = "<p>" + errorMsg[i] + "</p>";
                    $(form).find(".error-msgs").append(pelem);
                  }
                  $(form).find(".error-msgs").fadeIn(200);
                }
              }
            }).finally(function () {
              $(form).find("[type = 'submit']").removeClass("disable-events");
            })
          }
        }
      });
    }
  }
  //validate register form
  Auth.prototype.validateRegister = function () {
    if ($('#signup-form').length) {
      $('#signup-form').validate({
        focusInvalid: false,
        rules: {
          'username': {
            required: true
          },
          'email': {
            required: true,
            email: true,
            mailinatorDomain: true
          },
          'password': {
            required: true,
            minlength: 6,
            maxlength: 32,
          },
          'password_confirmation': {
            required: true,
            equalTo: "#password_primary"
          },
          'validation-required': {
            required: true
          }
        },
        errorPlacement: function errorPlacement(error, element) {
          if (error[0].innerText === 'Please enter a valid email address.') {
            $(element).siblings(".validation-error").text(error[0].innerText).removeClass("d-none");
          }
          else if (error[0].innerText === 'Please use valid email address.') {
            $(element).siblings(".validation-error").text("Please use correct email address").removeClass("d-none");
          }
          else if ($(error[0]).text() === 'Please enter the same value again.') {
            $(element).siblings(".validation-error").removeClass("d-none").text("Password and confirm password should be same");
          }
          else if ($(error[0]).text() === 'Please enter at least 6 characters.' || $(error[0]).text() === 'Please enter no more than 32 characters.') {
            $(element).siblings(".validation-error").removeClass("d-none").text($(error[0]).text());
          }
          else {
            $(element).siblings(".validation-error").addClass("d-none");
          }
          return true
        },
        highlight: function (element) {
          var $el = $(element);
          var $parent = $el.parents('.form-group');
          $parent.addClass("invalid-field")
        },
        unhighlight: function (element) {
          var $el = $(element);
          var $parent = $el.parents('.form-group');
          $parent.removeClass("invalid-field");
          $(element).siblings(".validation-error").addClass("d-none");
        },
        submitHandler: function (form) {
          $(form).find("[type = 'submit']").addClass("disable-events");
          if ($.API) {
            var prom = $.API.postWithoutToken(null, $(form).serializeArray(), registerRoute);
            prom.then(function (data) {
              $(form)[0].reset();
              window.location.reload()
            }).catch(function (error) {
              if (error && error.responseJSON) {
                var resp = error.responseJSON;
                if (resp.errors) {
                  var errorMsg = resp.errors;
                  $(form).find(".error-msgs p").remove();
                  for (var i in errorMsg) {
                    var pelem = "<p>" + errorMsg[i] + "</p>";
                    $(form).find(".error-msgs").append(pelem);
                  }
                  $(form).find(".error-msgs").fadeIn(200);
                }
              }
            }).finally(function () {
              $(form).find("[type = 'submit']").removeClass("disable-events");
            })
          }
        }
      });
      jQuery.validator.addMethod("mailinatorDomain", function(value, element) {
        if (value.search("mailinator.com") > -1) {
          return false;  // FAIL validation when REGEX matches
      } else {
          return true;   // PASS validation otherwise
      };
      }, "Please use valid email address.");
    }
  }
  //validate forgot form
  Auth.prototype.validateForgot = function () {
    if ($('#forgot-form').length) {
      $('#forgot-form').validate({
        focusInvalid: false,
        rules: {
          'email': {
            required: true,
            email: true
          },
          'validation-required': {
            required: true
          }
        },
        errorPlacement: function errorPlacement(error, element) {
          $(element).siblings(".validation-error").removeClass("d-none");
          return true
        },
        highlight: function (element) {
          var $el = $(element);
          var $parent = $el.parents('.form-group');
          $parent.addClass("invalid-field")
        },
        unhighlight: function (element) {
          var $el = $(element);
          var $parent = $el.parents('.form-group');
          $parent.removeClass("invalid-field");
          $(element).siblings(".validation-error").addClass("d-none");
        },
        submitHandler: function (form) {
          $(form).find("[type = 'submit']").addClass("disable-events");
          var payload = convertSerializeObj($(form).serializeArray())
          if ($.API) {
            var prom = $.API.postWithoutToken('user/forgot-password', payload, null);
            prom.then(function (data) {
              $(form)[0].reset();
              if (data.message === "User not found") {
                $(form).find(".success-msgs").find("p").remove();
                $(form).find(".success-msgs").removeClass("alert-success").addClass("alert-danger").append("<p>Email id not match with our records.</p>");
              }
              else if (data.code === 200) {
                $(form).find(".success-msgs").find("p").remove();
                $(form).find(".success-msgs").removeClass("alert-danger").addClass("alert-success").append("<p>Reset password mail has been send to your email address.</p>");
              }
              $(form).find(".success-msgs").fadeIn(200);
            }).catch(function (error) {
              if (err && err.responseJSON) {
                var resp = err.responseJSON;
                if ((resp.code && resp.code === 400) && resp.error) {
                  $(form).find(".error-msgs").append("<p>Email id not match with our records. Please contact admin in case your are facing any issue.</p>");
                  $(form).find(".error-msgs").fadeIn(200);
                }
              }
            }).finally(function () {
              $(form).find("[type = 'submit']").removeClass("disable-events");
            })
          }
        }
      });
    }
  }
  //validate reset form
  Auth.prototype.validateReset = function () {
    if ($('#reset-form').length) {
      $('#reset-form').validate({
        focusInvalid: false,
        rules: {
          'password': {
            required: true,
            minlength: 6,
            maxlength: 32
          },
          'password_confirmation': {
            required: true,
            equalTo: "#password_reset"
          },
          'validation-required': {
            required: true
          }
        },
        errorPlacement: function errorPlacement(error, element) {
          if ($(error[0]).text() === 'Please enter the same value again.') {
            $(element).siblings(".validation-error").removeClass("d-none").text("Password and confirm password should be same");
          }
          $(element).siblings(".validation-error").removeClass("d-none");
          return true
        },
        highlight: function (element) {
          var $el = $(element);
          var $parent = $el.parents('.form-group');
          $parent.addClass("invalid-field")
        },
        unhighlight: function (element) {
          var $el = $(element);
          var $parent = $el.parents('.form-group');
          $parent.removeClass("invalid-field");
          $(element).siblings(".validation-error").addClass("d-none");
        },
        submitHandler: function (form) {
          $(form).find("[type = 'submit']").addClass("disable-events");
          var param = new URLSearchParams(window.location.search);
          var token = param.get('token');
          var email = param.get('email');
          if ((email && email.length) && (token && token.length)) {
            var payload = convertSerializeObj($(form).serializeArray());
            payload.token = token;
            payload.email = email;
            if ($.API) {
              var prom = $.API.postWithoutToken('user/reset-password', payload);
              prom.then(function (data) {
                $(form)[0].reset();
                $(form).find(".success-msgs").append("<p>Password reset successfully. Please signin again.</p>");
                $(form).find(".success-msgs").fadeIn(200);
                setTimeout(function () {
                  window.location.reload();
                }, 5000)
              }).catch(function (error) {
                if (error && error.responseJSON) {
                  var resp = error.responseJSON;
                  $(form).find(".error-msgs p").remove();
                  if ((resp.code && resp.code === 400) && resp.error) {
                    $(form).find(".error-msgs").append("<p>" + resp.error.password + "</p>");
                    $(form).find(".error-msgs").fadeIn(200);
                  }
                }
              }).finally(function () {
                $(form).find("[type = 'submit']").removeClass("disable-events");
              })
            }
          }
        }
      });
    }
  }
  //init App
  $.Auth = new Auth, $.Auth.Constructor = Auth
}(window.jQuery),
  //initializing App
  function ($) {
    "use strict";
    $.Auth.init();
  }(window.jQuery);

function convertSerializeObj(formData) {
  var payload = {}
  formData.forEach(function (item) {
    payload[item.name] = item.value;
  })
  return payload
}

var pCount = 0;
function addProduct(arr, metaData) {
  if (arr.length) {
    var aggregateRating = 0;
    var productCount = 0;
    arr.forEach(function (item, index) {
      if(item.rating > 0){
        ++productCount;
        aggregateRating += item.rating
      }
    });
    var obj = {
      "@type": "product",
      "name": metaData.pageTitle && metaData.pageTitle.length ? metaData.pageTitle : "",
      "description": metaData.description,
      "aggregateRating": {
        "@type": "AggregateRating",
        "ratingValue": (aggregateRating/productCount).toFixed(2),
        "reviewCount": productCount
      },
      "offers": {
        "@type": "AggregateOffer",
        "availability": "https://lettstartdesign.com/InStock",
        "priceCurrency": "USD",
        "lowPrice": 4,
        "highPrice": 30,
        "offerCount": 1
      },
      "brand": "Lett Start Design",
      "review": []
    }
    return obj
  }
  return {};
}

function addArticle(name, desc, ldSchema, feature) {
  var obj = {
    "@type": "Article",
    "headline": name,
    "datePublished": "2021-02-11",
    "dateModified": "2021-02-11",
    "author": {
      "@type": "Organization",
      "name": "Lettstart Design"
    },
    "image": ["https://lettstartdesign.com/assets/images/home-work.svg"],
    "publisher": {
      "@type": "Organization",
      "name": "Google",
      "logo": {
        "@type": "ImageObject",
        "url": "https://lettstartdesign.com/assets/images/logo-dark.png"
      }
    },
    "@id": "https://lettstartdesign.com/#richSnippet",
    "isPartOf": {
      "@id": "https://lettstartdesign.com/#WebSite"
    },
    "inLanguage": "en-US",
    "mainEntityOfPage": {
      "@type": "WebPage",
      "@id": "https://lettstartdesign.com"
    }
  }
  if (desc) {
    obj["description"] = desc
  }
  ldSchema["@graph"].push(obj);
}

function faqSchema(faqData) {
  var arr = [];
  faqData.forEach(function (item, index) {
    var obj = {
      "@type": "Question",
      "name": item.question,
      "acceptedAnswer": {
        "@type": "Answer",
        "text": item.answer
      }
    }
    arr.push(obj);
  })
  return arr;
}

