!function ($) {
  ...
  PageProducts.prototype.init = function () {
    ...
    if (success === 'true') {
      ...
      sendGAEvent("Payment Status", "Payment Status", "Success");
    }
    else if (success === 'false') {
      $("#failureModal").modal('show');
      sendGAEvent("Payment Status", "Payment Status", "Failure");
    }
        ...
        if ($.API) {
          ...
          prom.then(function (data) {
            if(data && data.code === 200 && data.hasOwnProperty('product_name')) {
              ...
              sendGAEvent("Download Package", "Download Package", "Freebies");
            }
          }).catch(function (error) {
            if (error && error.noToken) {
              $("#loginModal").modal('show');
    ...
    if ($.API) {
      ...
      $.API.post('razorpay/order/verify', payload).then(function (resp) {
        if (resp && resp.code === 200 && resp.hasOwnProperty('product_name')) {
          self.downloadPackage()
          sendGAEvent("Download Package", "Download Package", "Razorpay");
        }
        else {
          $("#failureModal").modal('show');
        }
